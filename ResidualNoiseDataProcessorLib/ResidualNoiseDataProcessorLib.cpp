#include "ResidualNoiseDataProcessorLib.h"
...
namespace ResidualNoiseDataProcessor
{
	...
	bool read_meas_from_file(char* filename, uint32_t meas_num)
	{
		...
		if (file.is_open())
		{
			...
			//streampos pos = meas_num * config.data_size * 4;
			file.seekg(pos);
	...
	bool push_data_fixed_window_size(double* data_0, double* data_1)
	{
		...
		for (int b = (band_data.num_bands - 1); b >= 0; b--)
		{
			...
			for (int s = 0; s < band_data.bands[b].num_sets; s++)
			{
				...
				if (config.cross_corr)
				{
					//fft(smpl_buffer[0].data(), smpl_buffer[1].data(), fft_re_buffer.data(), fft_im_buffer.data());
				...
				{
					//rfft(smpl_buffer[0].data(), fft_re_buffer.data(), fft_im_buffer.data());
	...
	bool push_data_fixed_smpl_rate(double* data_0, double* data_1)
	{
		for (int b = (band_data.num_bands - 1); b >= 0; b--)
		{
			...
			for (int s = 0; s < band_data.bands[b].num_sets; s++)
			{
				...
				if (config.cross_corr)
				{
					//fft(data_0 + data_offset, data_1 + data_offset, fft_re_buffer.data(), fft_im_buffer.data());
				...
				{
					//rfft(data_0 + data_offset, fft_re_buffer.data(), fft_im_buffer.data());
